...
## Dev-Only Files — NEVER Commit Outside `dev`

The following files/folders are tracked **exclusively on the `dev` branch** for developer convenience (syncing configs across machines). They are gitignored everywhere else and must NEVER appear in commits, PRs, or pushes to `main` or any other branch:

| Path | Why it's dev-only |
|------|-------------------|
...
1. **Before staging any file**, run `git status` and verify none of the above paths are listed.
2. **Never use `git add .` or `git add -A`** — always stage files explicitly by name.
3. **Before creating a PR to `main`**, confirm the diff (`git diff main...HEAD`) contains zero dev-only files.
4. **If any dev-only file appears staged**, run `git restore --staged <file>` immediately before proceeding.
5. **PRs from `dev` to `main`** must never be created directly — feature branches must be cut from `dev`, with dev-only files excluded.
